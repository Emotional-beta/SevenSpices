...
public static class FinalPotSettlementTests
{
    ...
    static void Test_FinalPot_RealIngredients_AccumulateThenSettleAt32()
    {
        ...
        // F4 起 2 种味道再乘丰盛倍率 1.1，最终一次性乘 ×32 时在整体上向下取整。
        double totalBaseWithFlavor = state.Pot.BaseScoreWithFlavor;
        double abundance = ScoreCalculator.GetAbundanceMultiplier(state.Pot);
        run.EndCooking(MakeNormalCustomer());

        Assert(state.Pot.FinalScore == (int)Math.Floor(totalBaseWithFlavor * abundance * 32),
            $"最终锅应按「基础分{accumulated} + 味道分」合计 {totalBaseWithFlavor} × 丰盛{abundance} × 32 一次性结算");
    }
